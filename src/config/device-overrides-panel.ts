...
export class DeviceOverridesPanel extends LitElement {
  ...
  private async _confirmDeleteOverride(ev): Promise<void> {
    ...
    showConfirmationDialog(this, {
      text: html`${insteon.localize("utils.config_device_overrides.actions.confirm_delete")}<br />
        ${override.name}`,
      confirm: async () => await action(),
    });
  }

  private async _deleteOverride(hass: HomeAssistant, address: string): Promise<void> {
